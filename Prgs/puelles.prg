...
SCAN
	...
	SCAN
		...
		SCAN WHILE serie + nrodoc = cOs1.serie + cOs1.nrodoc
			...
			*SUSP
			lcItem = ""
			lnItem = 0 && último Item
			*** Buscando Item del Proceso en la Ficha Técnica
			SELECT ft1
			...
			***
			IF EMPTY(lcItem)
				*** Registrando nuevo Proceso en la Ficha Técnica
					...
					*** Buscando Guía de Remisión
					...
					SCAN WHILE docrel_tip + docrel_ser + docrel_nro + cod_articu = lcTipoOR + &lcTablaOR2..serie + &lcTablaOR2..nrodoc + &lcTablaOR2..cod_articu
						...
						IF !FOUND() OR &lcTablaGR1..cod_anula
							LOOP
						ENDIF
						*** Actualizando Ficha Técnica de la O/P
						SELECT ft3
